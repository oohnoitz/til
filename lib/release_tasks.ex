defmodule Til.ReleaseTasks do
  ...
  def grant_admin_permissions(email) do
    user = Til.Users.get_user_by_email(email)

    if user do
      case Til.Users.grant_user_admin_permissions(user) do
        {:ok, _} ->
  ...
  def revoke_admin_access(email) do
    ...
    if user do
      case Til.Users.revoke_user_admin_permissions(user) do
        {:ok, _} ->
